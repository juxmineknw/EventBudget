<template>
  <div class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4"
    @click="$emit('close')">
    <div class="bg-white w-full max-w-[420px] rounded-[32px] p-6 shadow-xl relative" @click.stop>
      <button @click="$emit('close')" class="absolute top-4 right-4 text-gray-400 text-2xl">
        ✕
      </button>

      <h2 class="text-xl font-bold mb-4">Edit Category</h2>
      <div class="rounded-2xl p-6 mb-4 shadow-inner" :style="{ backgroundColor: localColor }">
      ...
      <div class="flex gap-3 justify-center mb-4">
        <div v-for="c in colorOptions" :key="c" class="w-7 h-7 rounded-full border cursor-pointer"
          :style="{ backgroundColor: c }" @click="localColor = c">
          <div v-if="localColor === c" class="text-white text-center">✓</div>
        </div>
      </div>

      <input v-model="localName" class="w-full p-3 mb-3 bg-gray-100 rounded-xl" placeholder="Name" />

      <select v-model="selectedIconCategory" class="w-full p-3 mb-3 bg-gray-100 rounded-xl border">
      ...
      <div class="grid grid-cols-5 gap-3">
        <div v-for="ic in iconCategories[selectedIconCategory]" :key="ic" @click="localIcon = ic"
      ...
      <button @click="save" class="w-full py-3 mt-4 bg-[#FF7A1A] text-white rounded-xl font-semibold">
        Save
      </button>
    </div>
  </div>
