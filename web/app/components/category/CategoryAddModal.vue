...
<template>
  <div class="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center px-4 z-50"
    @click="emit('close')">
    <div class="bg-white rounded-[32px] p-6 max-w-[420px] w-full" @click.stop>
      <button class="absolute top-4 right-4 text-gray-400 text-2xl" @click="emit('close')">
        ✕
      </button>
      ...
      <div class="flex gap-3 justify-center mb-4">
        <div v-for="c in colorOptions" :key="c" class="w-7 h-7 rounded-full border cursor-pointer"
          :style="{ backgroundColor: c }" @click="color = c"></div>
      </div>

      <input v-model="name" placeholder="Name" class="w-full p-3 mb-3 bg-gray-100 rounded-xl" />

      <select v-model="selectedIconCategory" class="w-full p-3 mb-3 bg-gray-100 rounded-xl">
      ...
      <div class="grid grid-cols-5 gap-3">
        <div v-for="ic in iconCategories[selectedIconCategory]" :key="ic"
      ...
      <button class="w-full py-3 mt-4 bg-[#FF7A1A] text-white rounded-xl" @click="save">
        Confirm
      </button>
    </div>
  </div>
